...
## 2026-09-19 (rev 14) — model floor, native max effort, optional unsandboxed codex

- Files: `dist/local-agent-{targets,config,manager,catalog,codex,adapters,daemon-main}.js`, `behavior-tests/model-policy.mjs`, docs/configuration.md, docs/agent-profile-schema.md, `dist/fork-revision.js` (r13 → r14).
- What: (1) gpt-5.6-Terra and every gpt model below gpt-5.6 are refused at start/continue/persisted/profile with the message "Use gpt-5.6 models, e.g. gpt-5.6-luna with max thinking." (2) remove the earlier `max`→`xhigh` effort rewrite: `codex debug models` confirms gpt-5.6-luna natively advertises low/medium/high/xhigh/max and `max` is passed straight through (live turn agt_6837013d completed with stored effort "max"). (3) new per-provider `subagents.providers[].sandboxMode: "full-access"` forces Codex `danger-full-access` (no bwrap, no probe) for that provider; default `auto` keeps OS-sandbox selection. Deployed on nety3 with codex full-access (host bwrap cannot sandbox; see rev 11-13 posture — operator-explicit, not a fallback).
